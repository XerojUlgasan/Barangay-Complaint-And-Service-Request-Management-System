...
import { loginByEmail, checkHouseholdMember, registerByEmail, checkUserRole } from '../supabse_db/auth/auth';
import './login.css';
...
const Login = () => {
  ...
  const handleVerifySubmit = async (e) => {
    ...
    if (!result.isExist) {
      setVerifyError('No matching record found in barangay records. Please check your details.');
      return;
    }

    if (result.isActivated) {
      setVerifyError('This member already has an account. Please sign in instead.');
      return;
    }
    ...
    <div className="login-page">
      ...
      >
        ...
        <form onSubmit={handleSignInSubmit}>
          ...
          <button type="submit" className="btn btn-success" disabled={signInLoading}>
      ...
      >
        ...
        {step === 1 && (
          <div className="identity-section mt-4">
            ...
            <p className="text-center text-muted">Enter details matching barangay records</p>

            <form onSubmit={handleVerifySubmit}>
              <div className="mb-3">
                <label htmlFor="householdId" className="form-label">Household ID</label>
                <input type="text" className="form-control" id="householdId" placeholder="Enter your Household ID" value={householdId} onChange={(e) => setHouseholdId(e.target.value)} required />
              </div>
              <div className="mb-3">
              ...
              <div className="mb-3">
                <label htmlFor="middleName" className="form-label">
                  Middle Name <span className="text-muted" style={{ fontWeight: 400 }}>(optional)</span>
                </label>
                <input type="text" className="form-control" id="middleName" placeholder="Enter middle name" value={middleName} onChange={(e) => setMiddleName(e.target.value)} />
              </div>
              <div className="mb-3">
              ...
              <button type="submit" className="btn btn-success" disabled={verifyLoading}>
        ...
        {step === 2 && (
          <div className="identity-section mt-4">
            ...
            <p className="text-center text-muted">Create your login credentials</p>

            <form onSubmit={handleAccountSubmit}>
              <div className="mb-3">
                <label htmlFor="newEmail" className="form-label">Email</label>
                <input type="email" className="form-control" id="newEmail" placeholder="Enter your email" value={newEmail} onChange={(e) => setNewEmail(e.target.value)} required />
              </div>
              <div className="mb-3">
                <label htmlFor="newPassword" className="form-label">Password</label>
                <input type="password" className="form-control" id="newPassword" placeholder="Create a password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} required />
              </div>
              <div className="mb-3">
                <label htmlFor="confirmPassword" className="form-label">Confirm Password</label>
                <input type="password" className="form-control" id="confirmPassword" placeholder="Re-enter your password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} required />
              </div>
              ...
              <button type="button" className="btn btn-back mt-2" onClick={() => setStep(1)}>
                ← Back
              </button>
            </form>
          </div>
